...
// 扫描TS文件(异步)
exports.scanFileTsAsync = async function (scanPath) {
  const tsFiles = await glob.glob(path.join(process.cwd(), `${scanPath}/**/*.ts`));
  const tsxFiles = await glob.glob(path.join(process.cwd(), `${scanPath}/**/*.tsx`));
  // console.log(tsFiles);
  // console.log(tsxFiles);
  return tsFiles.concat(tsxFiles);
};
// 扫描VUE文件(异步)
exports.scanFileVueAsync = function (scanPath) {
  const entryFiles = glob.glob(
    path.join(process.cwd(), `${scanPath}/**/*.vue`)
  );
  // console.log(entryFiles);
  return entryFiles;
