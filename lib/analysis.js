...
const { scanFileTsAsync, scanFileVueAsync, getJsonContent } = require(
...
class CodeAnalysis {
  ...
  async _scanFiles(scanSource, type) {
    ...
    await Promise.allSettled(scanSource.map(async (item) => {
      const entryObj = {
        name: item.name,
      ...
      await Promise.allSettled(scanPath.map(async (sitem) => {
        let tempEntry = [];
        if (type === CODEFILETYPE.VUE) {
          tempEntry = await scanFileVueAsync(sitem);
        } else if (type === CODEFILETYPE.TS) {
          tempEntry = await scanFileTsAsync(sitem);
        }
